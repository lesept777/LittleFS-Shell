/*
  LittleFS_Shell.h - Library for LittleFS Shell.
...
#define LittleFSShell_FORMAT_LittleFS_IF_FAILED true
#define LittleFSShell_MAX_LINE_WIDTH 200
...
private:
  void static begin() {
    ...
    if (!LittleFS.begin(LittleFSShell_FORMAT_LittleFS_IF_FAILED)) {
      Serial.println("LittleFS Mount Failed");
      return;
    }

    Serial.println("Welcome to LittleFS Shell!\nType \"help\" to see available  commands");

    xTaskCreate(
        &(LittleFSShell::taskFunction),    // Task function
        "LittleFSTask",                  // Task name
  ...
  void static readFile(fs::FS &fs, const char *path, bool verbose = false) {
    ...
    while (file.available()) {
      ...
      while(c != '\n' && file.available()){
        c = file.read();
        str+= c;
        if(str.length() > LittleFSShell_MAX_LINE_WIDTH){
          str += "\n";
          break;
        }
      }
  ...
  void static dumpFile(fs::FS &fs, const char *path, int nmax = 16, bool verbose = false) {
    if(verbose) Serial.printf("Reading file: %s\r\n", path);
    ...
    while (file.available()) {
	  ...
	  if (n !=0) {
		for (int i = 0; i < 5*(nmax-n); ++i) Serial.print(' ');
		Serial.print('\t');
		Serial.print(str);
		Serial.println();
	  }
  ...
  void static printFlashSize() {
    uint64_t size = LittleFS.totalBytes() / 1024;
    Serial.printf("Flash memory size: %llu KB\n", size);
  }

  void static printFlashUsed() {
    uint64_t used = LittleFS.usedBytes() / 1024;
    Serial.printf("Flash memory used: %llu KB\n", used);
  }

  void static lsCommand(const String& dirname) {
    listDir(LittleFS, getAbsolutePath(dirname).c_str(), 0);
  }

  void static catCommand(const String& path) {
    readFile(LittleFS, getAbsolutePath(path).c_str());
  }

  void static dumpCommand(const String& path) {
    dumpFile(LittleFS, getAbsolutePath(path).c_str());
  }

  void static echoCommand(const String& message, const String& path) {
    writeFile(LittleFS, getAbsolutePath(path).c_str(), message.c_str());
  }

  void static appendCommand(const String& message, const String& path) {
    appendFile(LittleFS, getAbsolutePath(path).c_str(), message.c_str());
  }

  void static mvCommand(const String& path1, const String& path2) {
    renameFile(LittleFS, path1.c_str(), path2.c_str());
  }

  void static rmCommand(const String& path) {
    deleteFile(LittleFS, getAbsolutePath(path).c_str());
  }
  ...
  void static editorCommand(const String& path) {
    ...
    while (true) {
      ...
      if (input.equals("%exit")) {
        ...
        appendFile(LittleFS, getAbsolutePath(path).c_str(), input.c_str());
      }
    }
  }
  ...
  void static helpCommand() {
    ...
    Serial.println("dump <file> - Print the contents of a file using hexadecimal");
